...
function initCard() {
    ...
    var callback_friends_get = function(method,result,data){
        if(result) {
            ...
            FAPI.Client.call({"widget":"users.getInfo", "fields":"first_name,last_name", "uids":randomFriendId}, callback_users_getInfo);
        } else {
            processError(data);
        }
    }
    ...
    FAPI.Client.call({"fields":"first_name,last_name,location,pic128x128","widget":"users.getCurrentUser"}, callback_users_getCurrentUser);
    // пример №2: вызов метода без параметров
    FAPI.Client.call({"widget":"friends.get"}, callback_friends_get);
}
...
function publish() {
    ...
    feedPostingObject = {widget: 'stream.publish',
...
function checkSetStatusPermission(){
    ...
    FAPI.Client.call({"widget":"users.hasAppPermission", "ext_perm":permissionStatus}, callback);
}
...
function setUserStatus(){
    ...
    var params = {"widget":"users.setStatus", "status":status};
...
function showInvite2(){
    ...
    FAPI.Client.call({"widget":"friends.get"},callback);
}
